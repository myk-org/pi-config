...
1. Use the `review_status` tool to check review state. If unavailable, read `.pi/data/pi-config-review-state.jsonl` —
   this is a JSONL file (one JSON object per line); parse the **last non-empty line** to get current state.
   this is a JSONL file (one JSON object per line); parse the **last non-empty line** to get current state.
