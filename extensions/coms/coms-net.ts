 ...
 *
 * Drop-in successor to `coms-p2p.ts` whose substrate is a dedicated
...
// ━━ Helpers — verbatim from coms-p2p.ts (lines 131-210) ━━━━━━━━━━━━━━━━━━━━━━━━
...
export default function (pi: ExtensionAPI) {
	...
	function handleInboundPrompt(data: any): void {
		...
		const tasks: Array<{ subject: string; description: string }> | null = Array.isArray(data.tasks) ? data.tasks : null;
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (!serverUrl) {
			ctx.ui?.notify?.(
				`📡 coms-net: no server URL for project "${project}". Start one with: bun extensions/coms/coms-net-server.ts`,
				"error",
			);
		...
		try {
			...
			ctx.ui?.notify?.(
				...
				`Start one with: bun extensions/coms/coms-net-server.ts`,
				"error",
			);
