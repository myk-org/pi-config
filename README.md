...
`review_loop_enforcement: false`, not via max_cycles. Commit blocking is code-enforced via `isReviewClean`:
allows commit when `status: clean` + `tests_passed: true`, or when max cycles are exhausted
(`status: has_findings`, no pending reviewers, `tests_passed: true`, `cycle >= review_loop_max_cycles`).
