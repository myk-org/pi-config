...
| `review_loop_enforcement` | boolean | disabled | `PI_REVIEW_LOOP_ENFORCEMENT` | `true` automatically dispatches review agents and `test-runner` as the execution agent, and requires a clean review before commits. `tests_passed` is recorded only from detected test-command exit status, not agent completion. `false` disables automatic review-agent and test-agent dispatch; manual reviews/tests are optional and no review state, cycle, or result is required before commit. |
