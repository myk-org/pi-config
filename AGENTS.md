...
- Docs: auto-generated by docsfy — never edit `docs/` manually; never run docsfy unless the user explicitly asks
...
- 🚫 Never: `git add .`, `--no-verify`, commit to main/protected branches, edit `docs/` manually,
  run docsfy unless the user explicitly asks, add `pi install git:github.com/myk-org/pi-config`
...
**NEVER edit these files manually. NEVER run docsfy to regenerate them** unless the
user explicitly asks. Regeneration is not an agent task.
