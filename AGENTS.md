...
- Test: `tests/node/` (tsx + node:test), `tests/python/` (pytest), `packages/pi-sidecar/tests/` (sidecar TS + Python)
- Key packages: `@tintinweb/pi-tasks`, `pi-web-access`, `@myk-org/pi-sidecar`, `pi-sidecar-client`
