...
- **`bash`** (grep, find, rg) — Find error strings, trace function calls, locate patterns
...
For complex multi-component debugging, the orchestrator dispatches the debugger agent:

The debugger diagnoses only — it does not modify files.
The orchestrator delegates the actual fix to the appropriate language specialist.
